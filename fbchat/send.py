...
def _SEND_oldMessage(client, message):
    return message if isinstance(message, Message) else Message(text=message)


def _SEND_getSendData(client, message=None, thread_id=None, thread_type=ThreadType.USER):
...
def _SEND_doSendRequest(client, data, get_thread_id=False):
...
def SEND_send(client, message, thread_id=None, thread_type=ThreadType.USER):
    """
    ...
    data = _SEND_getSendData(
        message=message, thread_id=thread_id, thread_type=thread_type
    )

    return _SEND_doSendRequest(data)


def SEND_sendMessage(client, message, thread_id=None, thread_type=ThreadType.USER):
    """
...
def SEND_wave(client, wave_first=True, thread_id=None, thread_type=None):
    """
    ...
    """
    thread_id, thread_type = client._getThread(thread_id, thread_type)
    data = _SEND_getSendData(thread_id=thread_id, thread_type=thread_type)
    data["action_type"] = "ma-type:user-generated-message"
...
def SEND_quickReply(client, quick_reply, payload=None, thread_id=None, thread_type=None):
    """
...
def _SEND_sendLocation(client, location, current=True, thread_id=None, thread_type=None):
    thread_id, thread_type = client._getThread(thread_id, thread_type)
    data = _SEND_getSendData(thread_id=thread_id, thread_type=thread_type)
    data["action_type"] = "ma-type:user-generated-message"
...
def SEND_sendLocation(client, location, thread_id=None, thread_type=None):
    """
...
def SEND_sendPinnedLocation(client, location, thread_id=None, thread_type=None):
    """
...
def _SEND_upload(client, files, voice_clip=False):
    """
...
def _SEND_sendFiles(
        client, files, message=None, thread_id=None, thread_type=ThreadType.USER
):
    """
    ...
    thread_id, thread_type = client._getThread(thread_id, thread_type)
    data = _SEND_getSendData(
        message=_SEND_oldMessage(message),
        thread_id=thread_id,
        thread_type=thread_type,
    )
...
def SEND_sendRemoteFiles(
    ...
    files = _SEND_upload(get_files_from_urls(file_urls))
    return _SEND_sendFiles(
        files=files, message=message, thread_id=thread_id, thread_type=thread_type
    )


def SEND_sendLocalFiles(
    ...
    with get_files_from_paths(file_paths) as x:
        files = _SEND_upload(x)
    return _SEND_sendFiles(
        files=files, message=message, thread_id=thread_id, thread_type=thread_type
    )


def SEND_sendRemoteVoiceClips(
    ...
    files = _SEND_upload(get_files_from_urls(clip_urls), voice_clip=True)
    return _SEND_sendFiles(
        files=files, message=message, thread_id=thread_id, thread_type=thread_type
    )


def SEND_sendLocalVoiceClips(
    ...
    with get_files_from_paths(clip_paths) as x:
        files = _SEND_upload(x, voice_clip=True)
    return _SEND_sendFiles(
        files=files, message=message, thread_id=thread_id, thread_type=thread_type
    )
...
def SEND_createGroup(client, message, user_ids):
    """
    ...
    :raises: FBchatException if request failed
    """
    data = _SEND_getSendData(message=_SEND_oldMessage(message))
    ...
    message_id, thread_id = _SEND_doSendRequest(data, get_thread_id=True)
    if not thread_id:
        raise FBchatException(
...
def SEND_addUsersToGroup(client, user_ids, thread_id=None):
    """
    ...
    data = _SEND_getSendData(thread_id=thread_id, thread_type=ThreadType.GROUP)
...
def SEND_removeUserFromGroup(client, user_id, thread_id=None):
    """
...
def _SEND_adminStatus(client, admin_ids, admin, thread_id=None):
    thread_id, thread_type = client._getThread(thread_id, None)
...
def SEND_addGroupAdmins(client, admin_ids, thread_id=None):
    """
    ...
    :raises: FBchatException if request failed
    """
    _SEND_adminStatus(admin_ids, True, thread_id)


def SEND_removeGroupAdmins(client, admin_ids, thread_id=None):
    """
    ...
    :raises: FBchatException if request failed
    """
    _SEND_adminStatus(admin_ids, False, thread_id)


def SEND_changeGroupApprovalMode(client, require_admin_approval, thread_id=None):
    """
...
def _SEND_usersApproval(client, user_ids, approve, thread_id=None):
    thread_id, thread_type = client._getThread(thread_id, None)
...
def SEND_acceptUsersToGroup(client, user_ids, thread_id=None):
    """
    ...
    :raises: FBchatException if request failed
    """
    _SEND_usersApproval(user_ids, True, thread_id)


def SEND_denyUsersFromGroup(client, user_ids, thread_id=None):
    """
    ...
    :raises: FBchatException if request failed
    """
    _SEND_usersApproval(user_ids, False, thread_id)


def _SEND_changeGroupImage(client, image_id, thread_id=None):
    """
...
def SEND_changeGroupImageRemote(client, image_url, thread_id=None):
    """
    ...
    (image_id, mimetype), = _SEND_upload(get_files_from_urls([image_url]))
    return _SEND_changeGroupImage(image_id, thread_id)


def SEND_changeGroupImageLocal(client, image_path, thread_id=None):
    """
    ...
    with get_files_from_paths([image_path]) as files:
        (image_id, mimetype), = _SEND_upload(files)

    return _SEND_changeGroupImage(image_id, thread_id)


def SEND_changeThreadTitle(client, title, thread_id=None, thread_type=ThreadType.USER):
    """
...
def SEND_changeThreadColor(client, color, thread_id=None):
    """
...
def SEND_changeThreadEmoji(client, emoji, thread_id=None):
    """
    Changes thread color
...
def SEND_reactToMessage(client, message_id, reaction):
    """
...
def SEND_createPlan(client, plan, thread_id=None):
    """
    Sets a plan
...
def SEND_editPlan(client, plan, new_plan):
    """
    Edits a plan
...
def SEND_deletePlan(client, plan):
    """
    Deletes a plan
...
def SEND_changePlanParticipation(client, plan, take_part=True):
    """
...
def SEND_eventReminder(client, thread_id, time, title, location="", location_id=""):
    """
...
def SEND_createPoll(client, poll, thread_id=None):
    """
...
def SEND_updatePollVote(client, poll_id, option_ids=[], new_options=[]):
    """
    Updates a poll vote
...
def SEND_setTypingStatus(client, status, thread_id=None, thread_type=None):
    """
